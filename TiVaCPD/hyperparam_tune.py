...
def main():
    ...
    for i in range(12, len(X_samples)):
        print(i)
        print('--------------')
        ...
        if args.model_type == 'MMDATVGL_CPD':
            ...
            update_recall = 0
            update_peaks = []
            hyp_params = {'window_size':[5, 7, 10, 20],
                          'intermediate_dim_TD':[5, 10, 15, 20],
                          'intermediate_dim_FD':[5, 10, 15, 20],
                          'nfft':[30, 20, 10]}
            best_param = {}
            param_grid = ParameterGrid(hyp_params)
            random_param_set = random.choices(list(param_grid), k=30)

            f1_list = []
            num_epoch = 200
            for param_set in random_param_set:

                print(param_set)
                window_size = param_set['window_size']
                intermediate_dim_TD = param_set['intermediate_dim_TD']
                intermediate_dim_FD = param_set['intermediate_dim_FD']
                nfft = param_set['nfft']
                ...
                print("AUC:",np.round(metrics.auc,2), "F1:",np.round(metrics.f1,2), "Precision:", np.round(metrics.precision,2), "Recall:",np.round(metrics.recall,2))
                if metrics.f1 > best_dev_f1_score:
                    best_dev_f1_score = metrics.f1
                    update_auc = metrics.auc
                    update_precision = metrics.precision
                    update_recall = metrics.recall
                    update_peaks = metrics.peaks
                    best_param = param_set
            auc_scores.append(update_auc)
            f1_scores.append(best_dev_f1_score) 
            precision_scores.append(update_precision)
            recall_scores.append(update_recall)
            peaks = update_peaks
            print("AUC:",np.round(update_auc,2), "F1:",np.round(best_dev_f1_score,2), "Precision:", np.round(update_precision,2), "Recall:",np.round(update_recall,2))
            with open(os.path.join(data_path, ''.join(['AUTOENCODER_score_', str(i), '.txt'])), 'w') as writer:
                writer.write('{')
                writer.write(f"AUC: {np.round(update_auc,2)[0]}, F1: {np.round(best_dev_f1_score,2)}, Precision: {np.round(update_precision,2)}, Recall: {np.round(update_recall,2)}")
                writer.write('}'+ '\n')
                writer.write(f"{best_param}")
            # ############ HYPERPARAMETERS ###################
            # window_size = 7
